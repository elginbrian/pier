...
const CreateProposalPage: React.FC = () => {
  ...
  return (
    <>
          ...
          <div className="space-y-6">
              ...
              <FormSection stepNumber={1} title="General Information" icon="/info.svg">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormField label="Company Name" required>
                    <Input type="text" value={formData.companyName} onChange={handleInputChange("companyName")} placeholder="PT Vendor Teknologi" error={errors.companyName} />
                  </FormField>

                  <FormField label="Contact Person" required>
                    <Input type="text" value={formData.contactPerson} onChange={handleInputChange("contactPerson")} placeholder="Name of Person in Charge" error={errors.contactPerson} />
                  </FormField>

                  <div className="md:col-span-2">
                    <FormField label="Proposal Title" required>
                      <SmartTextarea value={formData.proposalTitle} onChange={handleInputChange("proposalTitle")} placeholder="Enter a clear and descriptive proposal title" rows={3} error={errors.proposalTitle} />
                    </FormField>
                  </div>

                  <FormField label="Service Type" required>
                    <Dropdown options={serviceOptions} onSelect={(option) => handleDropdownChange("serviceType")(option)} label="Select service type" error={errors.serviceType} />
                  </FormField>
                </div>
              </FormSection>
            </DashboardCard>
              ...
              <FormSection stepNumber={2} title="Service Details" icon="/detail-service.svg">
                <div className="space-y-6">
                  <FormField label="Technical Specification" required>
                    <SmartTextarea value={formData.technicalSpec} onChange={handleInputChange("technicalSpec")} placeholder="Describe the technical specifications of the service being offered..." rows={6} error={errors.technicalSpec} />
                  </FormField>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField label="Start Date" required>
                      <input
                        ...
                        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black ${
                          errors.startDate ? "border-red-300" : formData.startDate ? "border-green-300" : "border-gray-300"
                        }`}
                    ...
                    <FormField label="End Date" required>
                      <input
                        type="date"
                        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors text-black ${
                          errors.endDate ? "border-red-300" : formData.endDate ? "border-green-300" : "border-gray-300"
                        }`}
              ...
              <FormSection stepNumber={3} title="Budget & Costs" icon="/dollar.svg">
                <div className="space-y-6">
                  <FormField label="Contract Value (Rp)" required>
                    <Input type="number" value={formData.contractValue} onChange={handleInputChange("contractValue")} placeholder="0" error={errors.contractValue} />
                  </FormField>

                  <FormField label="Payment Terms Details" required>
                    <SmartTextarea value={formData.paymentTerms} onChange={handleInputChange("paymentTerms")} placeholder="Describe the payment term breakdown..." rows={4} error={errors.paymentTerms} />
                  </FormField>

                  <LargeFileUpload label="Upload Proposal Harga" required onFileSelect={handleFileChange("proposalHarga")} file={files.proposalHarga} error={errors.proposalHarga} />
                </div>
              </FormSection>
            </DashboardCard>
            ...
            <DashboardCard>
              <FormSection stepNumber={4} title="Supporting Documents">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <ProposalFileUpload label="Company Deed" onFileSelect={handleFileChange("companyDeed")} file={files.companyDeed} />

                  <ProposalFileUpload label="Business License" onFileSelect={handleFileChange("businessLicense")} file={files.businessLicense} />

                  <ProposalFileUpload label="Portfolio" onFileSelect={handleFileChange("portfolio")} file={files.portfolio} />
                </div>
              </FormSection>
            </DashboardCard>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center">
              ...
              <div className="flex gap-3">
                <Button variant="secondary" size="md" onClick={handleSaveDraft} disabled={isSubmitting}>
                  Save Draft
                </Button>
      ...
      {/* Success Modal */}
      <FormSendSuccessModal isOpen={isSuccessModalOpen} onClose={handleCloseSuccessModal} onLogin={handleDashboardRedirect} />
