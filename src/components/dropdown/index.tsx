  ...
  return (
    <div className={`relative w-full max-w-md ${className}`}>
      ...
      >
        ...
        <svg
          className={`w-5 h-5 text-[#313144] transition-transform duration-200 ${
            isOpen ? 'rotate-180' : ''
          }`}
          fill="none"
          stroke="currentColor"
        ...
        <div className="absolute top-full left-0 right-0 bg-white border-2 border-t-0 border-blue-500 rounded-b-lg shadow-lg z-50 overflow-hidden">
          {options.map((option: string, index: number) => (
            <button
              ...
              className={`w-full px-4 py-3 text-left hover:bg-[#E1E2E5] focus:outline-none focus:bg-gray-100 transition-colors ${
                index === 0 ? 'bg-[#C4CBDF] text-[#313144]' : ''
